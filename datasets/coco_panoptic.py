...
class CocoPanoptic:
    ...
    def __getitem__(self, idx):
        ...
        ann_path = os.path.join(self.ann_folder, ann_info['file_name'])  # 找到掩码图片名
        ...
        if "segments_info" in ann_info:
            masks = np.asarray(Image.open(ann_path), dtype=np.uint32)  # uint8用于存储各种图像的，范围是0-255，uint32可能是为了扩大范围
            masks = rgb2id(masks)
